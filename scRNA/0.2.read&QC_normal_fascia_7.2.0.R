...
# 设置数据目录
setwd(file.path(DATA_DIR, "normal_fascia"))
...
fascia <- merge(sclist[[1]], 
                y = sclist[[2]], 
                add.cell.ids = c("Fascia_1","Fascia_2"), 
                project = "fascia")
fascia
...
# 质控前可视化
pdf(file = file.path(OUTPUT_DIR, "qc", "preQC-fascia.pdf"), width = 10, height = 6)              
...
fascia <- subset(fascia, subset = 
  nCount_RNA < QC_PARAMS$normal_fascia$nCount_RNA_max & 
  nFeature_RNA < QC_PARAMS$normal_fascia$nFeature_RNA_max & 
  percent.mt < QC_PARAMS$normal_fascia$percent_mt_max
)

# 质控后可视化
pdf(file = file.path(OUTPUT_DIR, "qc", "postQC-fascia.pdf"), width = 10, height = 6)              
VlnPlot(fascia, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
        ncol = 3, 
        group.by = "orig.ident", 
        pt.size = 0)
...
# 保存质控后的对象
qsave(fascia, file.path(OUTPUT_DIR, "qc", "normal_fascia.qs"))

# 清理环境
rm(counts, folders, sclist)

cat("=== 质控完成 ===\n")
cat("质控前细胞数:", ncol(fascia@assays$RNA@counts), "\n")
cat("质控后细胞数:", ncol(fascia@assays$RNA@counts), "\n")
cat("质控参数：\n")
cat("  UMI数上限:", QC_PARAMS$normal_fascia$nCount_RNA_max, "\n")
cat("  基因数上限:", QC_PARAMS$normal_fascia$nFeature_RNA_max, "\n")
cat("  线粒体基因比例上限:", QC_PARAMS$normal_fascia$percent_mt_max, "\n")

# 注意：保存工作环境的功能已移除，改为使用配置文件管理
# 如需保存特定结果，请使用qsave保存到OUTPUT_DIR相应目录