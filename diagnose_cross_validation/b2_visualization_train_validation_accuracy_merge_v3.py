    ...
    if os.path.exists(csv_file):
        ...
        for epoch in range(1, 101):
            ...
            if accuracy > best_accuracy:
                ...
                early_stopping_counter = 0
                if best_accuracy > 99:
                    torch.save(model.state_dict(), f'b2_{os.path.basename(csv_file)}_{best_accuracy:.2f}.pth')
            ...
            if early_stopping_counter > 9:
                ...
                print(f"Completed Epoch {epoch} for {folder_name}")
                break
        model_save_path = f"b2_{folder_name}_accuracy_{best_accuracy:.2f}_model.pth"
...
plt.tight_layout()
plt.savefig('b2_all_models_training_visualization_legend.pdf')
plt.close()
...
plt.tight_layout()
plt.savefig('b2_all_models_training_visualization.pdf')
plt.close()

training_records_df = pd.DataFrame(training_records, columns=['Folder_Name', 'Epoch', 'Accuracy'])
training_records_df.to_csv('b2_training_records.csv', index=False)